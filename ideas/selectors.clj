...
(defn ^Boolean string-literal?
  [^String s]
  (boolean (re-matches string-literal-re s)))


(defn ^Boolean string-contains?
  [^String s1 ^String s2]
  (.contains s1 s2))
...
(defn parse-seq-as-space-separated-list
  [v]
  (reify
    IParse
    (parse
...
(defn parse-seq-as-comma-separated-list
  [v]
  (reify
    IParse
    (parse
